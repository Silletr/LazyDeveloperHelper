...
def install_requirements(search_path="."):
    for dirpath, _, filenames in os.walk(search_path):
        if REQUIREMENTS_FILE in filenames:
            ...
            try:
                run(["pip", "install", "-r", os.path.join(dirpath, REQUIREMENTS_FILE)],
                    check=True)
                return True
            ...
            log_message("requirements.txt not found, using requirements-dev.txt", "info")
            try:
                run(["pip", "install", "-r", os.path.join(dirpath, ALTERNATIVE_FILE)],
                    check=True)
                return True
            except CalledProcessError as e:
                log_message(f"Error installing requirements-dev.txt: {e}", "error")
                return False
    log_message(f"{REQUIREMENTS_FILE} or {ALTERNATIVE_FILE} not found in {search_path}!", "error")
    return False
